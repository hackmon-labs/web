...
export default function RpgModal() {
  ...
  const { runAsync: attackFn, loading: attackLoading } = useRequest(
    () =>
      fethchFn({
        url: ATTACK_URL,
        token: info?.token,
  ...
  const { runAsync: attackStartFn, loading: startLoading } = useRequest(
    () =>
      fethchFn({
        url: ATTACKSTRAT_URL,
        token: info?.token,
  ...
  const { runAsync: recovertFn } = useRequest(
    () =>
      fethchFn({
        url: RECOVER_URL,
        token: info?.token,
  ...
  const winMessage = (item) => {
    const award =
      item.win == 'hacker' ? `gold:${item.gold},box:${item.box}` : '';
    messageApi.open({
      key,
      type: 'success',
  ...
  return (
    <div className="rpgBox">
      ...
      >
        ...
        >
          {monster && (
            <div className="monsterBox" style={{ marginBottom: 20 }}>
              <div>
                ...
                <div className="font20 width400">
                  <Progress
                    ...
                    showInfo={false}
                    format={(percent) =>
                      percent + '/' + monster.monsterMaxHp + 'hp'
                    }
                    percent={(percentMonster / monster.monsterMaxHp) * 100}
                    status="active"
                  />
                  <span>
                    HP : {percentMonster}/{monster.monsterMaxHp}
                  </span>
                </div>
                <div className="font20 ">
                  ATK : {monster.monsterMinDamage} - {monster.monsterMaxDamage}
                </div>
              </div>

              <div className="monsterPng">
                <img src={MonsterPng.src} />
              </div>
            </div>
          )}
          {user && (
            <div className="hackerBox">
              <div className="monsterPng">
                <img src={HackerPng.src} style={{ height: 256 }} />
              </div>
              <div className="ml50">
                ...
                <div className="font20 width500">
                  <Progress
                    showInfo={false}
                    percent={
                      (percentHacker / (user.maxHp + user.addMaxHp)) * 100
                    }
                    status="active"
                  />
                  <span>
                    HP : {percentHacker}/{user.maxHp + user.addMaxHp}{' '}
                  </span>
                </div>
                <div className="font20 ">
                  ATK : {user.minDamage + user.addMaxAtk} -{' '}
                  {user.maxDamage + user.addMaxAtk}
                </div>

                <div className="action ">
                  <div onClick={attack} className="font20 actionItem1">
                    attack
                  </div>
                  <div
                    onClick={recovert}
                    className="font20 actionItem1"
                    style={{ color: 'blue' }}
                  >
                    recover {user.blood}
                  </div>
                  <div onClick={handleCancel} className="font20 actionItem2">
                    Run away
                  </div>
                </div>
              </div>
