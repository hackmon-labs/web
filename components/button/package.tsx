...
export default function PackageModal() {
  ...
  const { runAsync: getInfo, loading: attackLoading } = useRequest(
    () =>
      fethchFn({
        url: ATTACKSTRAT_URL,
        token: info?.token,
  ...
  const { runAsync: getNFTS, loading: nftsLoading } = useRequest(
    () =>
      fethchFn({
        url: GETNFTS_URL,
        token: info?.token,
  ...
  const {
    ...
    () =>
      fethchFn({
        url: UPDATEITEM_URL,
        token: info?.token,
        tokens: checkedValues?.map((item) =>
          nfts.find((j) => item == j.tokenId)
        ),
      }),
    {
      manual: true,
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 30, color: 'gold' }}>
              ...
              <span className="boxOnChain" onClick={boxOnChain}>
                Sync to the chain
              </span>
            </div>
            <div style={{ fontSize: 30, color: 'gold' }}>
              box(chain):{readData}
              <span className="boxOnChain" onClick={openChainBox}>
                open NFT
              </span>
            </div>
            <div style={{ fontSize: 30, color: 'gold' }}>
              blood:{infoDetail?.blood}
              <span className="boxOnChain" onClick={buyBloodFn}>
                buy(100gold)
              </span>
            </div>
                ...
                {nfts?.map((nft) => (
                  <div>
                    ...
                    >
                      <Meta
                        title={nft.title}
                        description={nft?.rawMetadata?.attributes?.map(
